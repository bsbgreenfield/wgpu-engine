...
mod scene_tests {
    ...
    #[test]
    fn sweep_reports_the_weakest_asset_as_the_achieved_level() {
        let mut fixture = Fixture::new();
        let mut graph = DependencyGraph::default();

        let resident = fixture.gpu();
        let half = fixture.cpu();
        let cold = fixture.unloaded();

        let all_resident = fixture.raw_scene(&[resident], &[]);
        let mixed = fixture.raw_scene(&[resident, half], &[]);
        let blocked = fixture.raw_scene(&[half, cold], &[]);
        let empty = fixture.raw_scene(&[], &[]);
        for scene in [&all_resident, &mixed, &blocked, &empty] {
            graph.add_scene(scene, &fixture.entities).expect("added");
        }

        let result = graph
            .sweep(
                (0..4).map(|i| (SceneId(i), SceneLoadLevel::GPU)),
                &fixture.assets,
            )
            .expect("sweep");

        let achieved = result.achieved_scenes;
        assert_eq!(achieved[&SceneId(0)], SceneLoadLevel::GPU);
        assert_eq!(
            achieved[&SceneId(1)],
            SceneLoadLevel::CPU,
            "one CPU-only asset holds the whole scene at CPU"
        );
        assert_eq!(achieved[&SceneId(2)], SceneLoadLevel::NotLoaded);
        assert_eq!(
            achieved[&SceneId(3)],
            SceneLoadLevel::GPU,
            "a scene with no assets is trivially as loaded as it can get"
        );
    }

    #[test]
    fn sweep_requires_the_highest_level_any_holder_asked_for() {
        let mut fixture = Fixture::new();
        let mut graph = DependencyGraph::default();
        ...
        let modest = fixture.raw_scene(&[shared], &[]);
        let greedy = fixture.raw_scene(&[shared, solo], &[]);
        graph.add_scene(&modest, &fixture.entities).expect("added");
        graph.add_scene(&greedy, &fixture.entities).expect("added");

        let result = graph
            .sweep(
                [
                    (SceneId(0), SceneLoadLevel::CPU),
                    (SceneId(1), SceneLoadLevel::GPU),
                ]
                .into_iter(),
                &fixture.assets,
            )
        ...
        assert_eq!(
            result.required_assets[&shared],
            SceneLoadLevel::GPU,
            "the GPU request must win over the CPU request for the shared asset"
        );
        assert_eq!(result.required_assets[&solo], SceneLoadLevel::GPU);

        let missing = graph.sweep(
            [(SceneId(9), SceneLoadLevel::CPU)].into_iter(),
            &fixture.assets,
        );
        assert!(matches!(missing, Err(DependencyGraphError::SceneNotFound)));
    }
